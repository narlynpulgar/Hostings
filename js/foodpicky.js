...
$(document).ready(function() {
    ...
    headroom.init();
    ////////Packages filter
    var $container = $(".LiveStock-listing");
    $container.isotope({
        filter: "*",
        animationOptions: {
            duration: 750,
    ...
    $("#slider-range").slider({
        ...
        slide: function(event, ui) {
            //$( "#amount" ).val( "PHP" + ui.values[ 0 ] + " - $" + ui.values[ 1 ] );
            $(".minvalue").html("PHP" + ui.values[0]);
            $(".maxvalue").html("PHP" + ui.values[1]);
        }
    });
    $("#amount").val("PHP" + $("#slider-range").slider("values", 0) + " - $" + $("#slider-range").slider("values", 1));
